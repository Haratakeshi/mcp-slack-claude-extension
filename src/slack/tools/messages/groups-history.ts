...
// 入力スキーマを定義
export const GroupsHistorySchema = MessageHistorySchema;

export type GroupsHistoryInput = z.infer<typeof GroupsHistorySchema>;

/**
 * プライベートチャンネル（グループ）のメッセージ履歴を取得します。
 * Slack APIの `conversations.history` メソッドに対応します。
 */
export const groupsHistoryDescription = 'プライベートチャンネル（グループ）のメッセージ履歴を取得します。';

/**
 * @param env {ToolEnv} SLACK_USER_TOKENを含む環境変数
 * @param input {GroupsHistoryInput} ツールへの入力
 * @returns {Promise<object>} メッセージ履歴とページネーション情報
 */
export async function groupsHistory(env: ToolEnv, input: GroupsHistoryInput): Promise<object> {
  // 1. 入力検証
  const validatedInput = GroupsHistorySchema.parse(input);
